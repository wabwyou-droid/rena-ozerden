...
<script>
  ...
  function connect(){
    ...
    ws.onmessage=function(e){
      ...
      if(msg.type==='init'){msg.dilekler.forEach(function(d){spawnNote(d.isim,d.mesaj,d.foto);});count=msg.dilekler.length;document.getElementById('ct').textContent=count+' not';}
      else if(msg.type==='dilek'){spawnNote(msg.dilek.isim,msg.dilek.mesaj,msg.dilek.foto);}
    };
...
<` + `/script>`;
  ...
  const finalBody = bodyHtml.replace('<canvas id="qrCanvas" style="display:block;width:100%;height:100%;"></canvas>',
